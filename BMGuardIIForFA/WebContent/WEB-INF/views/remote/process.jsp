...
<c:set var="tab_1d" value="프로세스 실행" />
...
<nav class="tab_1depth">
	<ul>
		<li class="on"><a href="./process.html">프로세스 실행</a></li>
			...
			<tbody>
				<tr>
					...
					<td class="detail" style="border: 0px;">
						...
						<br/><br/>※ 실행할 프로세스의 Full Path 입력 [예) C:\prog\cqnoti.exe]
					</td>
				</tr>
				<tr>
					<th><label for="sau">프로세스 실행사유</label></th>
					<td class="detail" style="border: 0px;">
	 ...
	 $("#tree_device_list a").live("click", function(e) {
		...
		$("#submitBtn").click(function() {  
			...
			if(process.trim().length==0) {
				alert("실행할 프로세스명을 입력하여 주세요.");
				return;
			}
			if(reason.trim().length ==0) {
				alert("프로세스 실행사유를 입력하여 주세요.");
				return;
			}
			//전송할 device idx, ip
			...
			if(device_idxs.length == 0) {
				alert("프로세스를 실행할 자동화 기기를 선택하여 주세요.");
				return;
			}
			...
			/* document.BMSupoAx.ExecProcess (IP, port, 실행할 프로세스, 클라이언트 idx)*/
			for(var i = 0 ; i < device_idxs.length;i++) {
				...
				document.BMSupoAx.ExecProcess( device_ip, 7997, process, device_idx);

...
<c:set var="tab_1d" value="레지스트리 수정" />
...
<nav class="tab_1depth">
	<ul>
		<li><a href="./registry.html">레지스트리 조회</a></li>
		<li><a href="./registry2.html">레지스트리 추가</a></li>
		<li class="on"><a href="./registry3.html">레지스트리 수정</a></li>
		...
		<div class="table_body mt0">
			<table class="mt20 sub_table">
				...
				<tbody>
					...
					<tr>
						<th>값</th>
						<td colspan="3"><input type="text" id="data_val" name="data_val" style="width: 664px;"></td>
					</tr>					
					<tr>
						<th>레지스트리 수정 사유</th>
						<td colspan="3"><input type="text" name="reason" id="reason" style="width: 664px;"/></td>
					</tr>
				</tbody>
			</table>
...
$(document).ready(function() {
	...
	$("#submitBtn").click(function() {  
		...
		if(data_val.trim().length==0) {
			alert("데이터 값을 입력하여 주세요.");
			return;
		}
		if(reason.trim().length==0) {
			alert("레지스트리 수정 사유을 선택하여 주세요.");
			return;
		}
		...
		if(device_idxs.length == 0) {
			alert("레지스트리를 수정할 자동화 기기를 선택하여 주세요.");
			return;
		}
		...
		/* document.BMSupoAx.UpdateRegistry (IP, port, 키, 서브키,데이터 타입, 값이름, 데이터값,  클라이언트 idx)*/
...
/* 			if(device_ip == "" || device_ip == "null") {
			...
			alert("("+device_ip +", 7997 ,"+ key +", "+ sub_key +", "+ data_type +", "+ data_name +", "+data_val +", "+device_idx+" )");
			document.BMSupoAx.UpdateRegistry( device_ip, 7997, key, sub_key, data_type, data_name, data_val, device_idx);

...
<nav class="tab_1depth">
	<ul>
		...
		<li class="on"><a href="./file4.html">폴더생성</a></li>
	</ul>
</nav>
</header>
...
<article>
	...
	<div class="board">
		...
		<table>
			...
			<tbody>
				<tr>
					<th><label for="forname">생성할 폴더명</label></th>
					<td class="detail" style="border: 0px;">
						<input type="text" id="folderpath" name="folderpath" style="width:630px"/>
						<input type="hidden" id="job_idx" name="job_idx" />
						<br/><br/><span>※ 생성할 폴더명의 Full Path 입력[예)c:\New Folder]</span>
					</td>
				</tr>
				<tr>
					<th><label for="sau">폴더 생성 사유</label></th>
					<td class="detail" style="border: 0px;">
	 ...
	 $("#tree_device_list a").live("click", function(e) {
		...
		$("#submitBtn").click(function() {  
			...
			if(folderpath.trim().length==0) {
				alert("생성할 폴더명을 입력하여 주세요.");
				return;
			}
			if(reason.trim().length ==0) {
				alert("폴더 생성 사유를 입력하여 주세요.");
				return;
			}
			//전송할 device idx, ip
			...
			if(device_idxs.length == 0) {
				alert("폴더를 생성할 자동화 기기를 선택하여 주세요.");
				return;
			}
			...
			/* document.BMSupoAx.CreateFolder(IP, port, 폴더 경로 , 클라이언트 idx)*/
 ...
 */				alert("("+device_ip +", 7997 ,"+folderpath+", "+device_idx+" )");
				document.BMSupoAx.CreateFolder( device_ip, 7997, folderpath, device_idx);

...
<c:set var="tab_1d" value="프로세스 종료" />
...
<nav class="tab_1depth">
	<ul>
		...
		<li class="on"><a href="./process2.html">프로세스 종료</a></li>
		<li><a href="./process3.html">서비스 상태검사</a></li>
		<li><a href="./process4.html">서비스 시작</a></li>
		<li><a href="./process5.html">서비스 중지</a></li>
	</ul>
</nav>
</header>
			...
			<tbody>
				<tr>
					...
					<td class="detail" style="border: 0px;">
						<input type="text" style="width:630px" id="process" name="process" />
						<input type="hidden" id="job_idx" name="job_idx" />
						<br/><br/><span>※ 종료시킬 프로세스 이름을 작업관리자에 표시되는 이름으로 입력[예) MyProcess.exe]</span>

					</td>
				</tr>
				<tr>
					<th><label for="sau">프로세스 종료사유</label></th>
					<td class="detail" style="border: 0px;">
	 ...
	 $("#tree_device_list a").live("click", function(e) {
		...
		$("#submitBtn").click(function() {  
			...
			if(process.trim().length==0) {
				alert("종료할 프로세스명을 입력하여 주세요.");
				return;
			}
			if(reason.trim().length ==0) {
				alert("프로세스 종료 사유를 입력하여 주세요.");
				return;
			}
			//전송할 device idx, ip
			...
			if(device_idxs.length == 0) {
				alert("프로세스를 종료할 자동화 기기를 선택하여 주세요.");
				return;
			}
			...
			/* document.BMSupoAx.KillProcess(IP, port, 종료할 프로세스 , 클라이언트 idx)*/
...
/* 				if(device_ip == "" || device_ip == "null") {
				...
				alert("("+device_ip +", 7997 ,"+process+", "+device_idx+" )");
				document.BMSupoAx.KillProcess( device_ip, 7997, process, device_idx);

...
/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* 관리그룹/OS타입 관련 공통 스크립트                                                                                             */
/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
//관리그룹이 하나이상 선택되었는지 체크합니다.
function checkManageGroupSelected(manageGroupId){
	if($(manageGroupId + " li :checkbox:checked").size() == 0){
		alert("관리그룹은 하나 이상 선택하여야 합니다.");
		return;
	}
}

function setOstype(ostypeId, formId){
	
}

// 관리그룹 체크박스 선택시 값을 form에 반영합니다.
function setManageGroupIdx(manageGroupId, formId){
	
	$(manageGroupId+ " li :checkbox").click(function(){
		var groupIdx = $(this).val();
		if(this.checked){
			if(groupIdx != ""){
				$(formId).append("<input type='hidden' name='clientGroupIdx' value='"+groupIdx+"'>");
			}else{
				$(formId +" [name='clientGroupIdx']").remove();
				$(manageGroupId + " li :checkbox").each(function(index, el){
					el.checked = true;
					if($(el).val() != ""){
						$(formId).append("<input type='hidden' name='clientGroupIdx' value='"+$(el).val()+"'>");
					}
				});					
			}
		}else{
			if(groupIdx != ""){
				$(manageGroupId+ " li :checkbox")[0].checked = false;
				$(formId + " [name='clientGroupIdx']").each(function(index, el){
					if($(el).val() == groupIdx){
						$(el).remove();
					}
				});
			}else{
				clearManageGroupChecked(manageGroupId, formId);
			}
		}
	});
}
...
// 관리그룹 선택을 해제합니다.
function clearManageGroupChecked(manageGroupId, formId){
	$(manageGroupId + " li :checkbox").each(function(index, el){
		el.checked = false;
	});
	$(formId+ " [name='clientGroupIdx']").remove();
...
// 해당 월의 첫 날짜 얻기
function getBeginDateAtMonth(year, month){
	return getDateFormat(new Date(year, month -1));
}

// 해당 월의 마지막 날짜 얻기
function getLastDateAtMonth(year, month){
	
	var d = new Date(); 
	d.setTime(new Date(year, month) - 1); 
	return getDateFormat(d);
}

//현재날짜에서 period에 해당하는 기간만큼의 이전날짜를 조회한다.
function getDateStringByPeriod(now, period){		
	if(period != null){
...
function getDateStringToLocalDateFormat(date_string){
	var dateArray = date_string.split("-");
	var year = dateArray[0];
	var month = dateArray[1];
	var day = dateArray[2];
	
	return getLocalDateFormat(new Date(year, month-1, day));
}
...
/*문자열을 byte수로 계산하여  몇자 이상 몇자 이내로 입력하는지 체크*/
function strRestMinMax(str, type, minN, maxN) {
	/** type == kor 2byte eng 1byte */
	...
	if (str.val().byte() >= minNum ) {
		if(str.val().byte() <=maxNum) {
			return true;
		}else {
			alert("글자수를 " + maxNum + "자 이내로 입력하여 주세요." );
			str.focus();
		}
	}else {
		alert("글자수를  " + minNum +"자 이상 입력하여 주세요." );
		str.focus();
	}
}
/* type 종류
 ...
 * kor_eng : 한/영 
 * ub_all :  특수문자-_
 * special_on :특수문자, 영문, 숫자 각 1자이상
 * number : 숫자
 */
function strRestType(type, value) {
	var result = "";
	var flag = false;
	if (type =="kor") {
		flag = /^[가-힣]+$/.test(value);
		...
		flag =   /^[가-힣a-zA-Z]+$/.test(value);
		if(!flag) {
			result ="한글과 영문만 입력가능합니다.";
		}
	}
	else if(type=="ub_all"){
		flag = /^[_a-zA-Z0-9-]+$/.test(value);
		if(!flag) {
			result = "특수문자 -_와 영문자/숫자만 입력가능 합니다.";
		}
	}
	else if(type=="special_one"){
		var re1 = /[a-zA-Z]/i; // 영문
		var re2 = /[0-9]/i; // 숫자
		var re3 = /[\~'@!#\$\^%&*()+=\-\[\]\\\';,\.\/\{\}\|\":<>\?]/i; // 특수기호
		...
		if (re1.test(value) && re2.test(value) && re3.test(value) && re4.test(value)) { 
			flag = true;
		}else {	result ="특수문자/영문/숫자 각 1자이상 포함하여야 합니다."; }
	}
	else if(type=="number"){
		flag = /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(value);
		if(!flag) {
			result = "숫자만 입력가능 합니다.";
		}
	}
...
function checkedAllCheckbox(all_checkbox_id, checkboxes_name){
	$("input[name="+checkboxes_name+"]:checkbox").click(function() {

...
function Show_cal(sYear, sMonth, sDay) {
	...
	if (intThisMonth == 0) intThisMonth = parseInt(datToday.getMonth(),10)+1;	// 월 값은 실제값 보다 -1 한 값이 되돌려 진다.
	...
	Cal_HTML += "<table id=Cal_Table border=0 bgcolor='#f4f4f4' cellpadding=1 cellspacing=1 width=100% onmouseover='parent.doOver(window.event.srcElement)' onmouseout='parent.doOut(window.event.srcElement)' style='font-size : 12;font-family:굴림;cursor:pointer;'>";
	Cal_HTML += "<tr height='35' align=center bgcolor='#f4f4f4'>";
	Cal_HTML += "<td colspan=7 align=center>";
	Cal_HTML += "	<select name='selYear' STYLE='font-size:11;color:#808080;border:1px solid #d5d5d5;' OnChange='parent.fnChangeYearD(calendar.selYear.value, calendar.selMonth.value, "+intThisDay+")';>";
	...
	Cal_HTML += "	</select>";
	Cal_HTML += "&nbsp;&nbsp;&nbsp;<a style='cursor:pointer;' OnClick='parent.Show_cal("+intPrevYear+","+intPrevMonth+","+intThisDay+");'><img src='../common/images/btn/btn_arrow_left.gif' /></a> ";
	Cal_HTML += "<select name='selMonth' STYLE='font-size:11;padding:1px 0;color:#808080;border:1px solid #d5d5d5;' OnChange='parent.fnChangeYearD(calendar.selYear.value, calendar.selMonth.value, "+intThisDay+")';>";
	for (var i=1; i<13; i++) {	
	...
	Cal_HTML += "	</select>&nbsp;";
	Cal_HTML += "<a style='cursor:pointer;' OnClick='parent.Show_cal("+intNextYear+","+intNextMonth+","+intThisDay+");'><img src='../common/images/btn/btn_arrow_right.gif' /></a>";
	Cal_HTML += "</td></tr>";
	Cal_HTML += "<tr style='color:#2065DA;font-size:11px; text-align:center;' >";
		...
		for (intLoopDay=1; intLoopDay <= 7; intLoopDay++) {	// 요일단위 루프 시작, 일요일 부터
			if (intThirdWeekday > 0) {											// 첫주 시작일이 1보다 크면
				...
				if (thirdPrintDay > intLastDay) {								// 입력 날짝 월말보다 크다면
					...
					Cal_HTML += "<td style='font-size:11px; text-align:center;' onClick=parent.Calendar_Click(this); title="+intThisYear+"-"+day2(intThisMonth).toString()+"-"+day2(thirdPrintDay).toString()+" style=\"cursor:pointer;border:1px solid white;";
	...
	oPopBody.style.border = "1px solid #d5d5d5";
	...
	oPopup.show(0, srcElement.offsetHeight, 170, calHeight, srcElement);
}
...
function Show_cal_M(sYear, sMonth) {
	...
	Cal_HTML += "<a style='cursor:pointer;' OnClick='parent.Show_cal_M("+intPPyear+","+intThisMonth+");'><img src='../common/images/btn/btn_arrow_left.gif' /></a>&nbsp;";
	Cal_HTML += "<select name='selYear' style='font-size:11px;color:#808080;border:1px solid #d5d5d5;' OnChange='parent.fnChangeYearM(this.value, "+intThisMonth+")';>";
	for (var optYear=(intThisYear-2); optYear<(intThisYear+2); optYear++) {
			Cal_HTML += "		<option value='"+optYear+"' ";
			if (optYear == intThisYear) Cal_HTML += " selected>\n";
			else Cal_HTML += ">\n";
	...
	Cal_HTML += "	</select>\n";
	Cal_HTML += "<a style='cursor:pointer;' OnClick='parent.Show_cal_M("+intNNyear+","+intThisMonth+");'><img src='../common/images/btn/btn_arrow_right.gif' /></a>";
	Cal_HTML += "</td></tr>\n";
